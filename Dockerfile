...
# Instala lsphp y los paquetes necesarios
...
# configura usuario jimenozki
RUN set -ex; \
    # crea el usuario jimenozki
    adduser --gecos '' --disabled-password jimenozki; \
...
RUN set -ex; \
    # sudoers
    echo "jimenozki ALL=(ALL) NOPASSWD:SETENV: /usr/local/lsws/bin/lswsctrl, /usr/local/bin/jimenozki-admin, /usr/local/bin/jimenozki-config, /usr/local/bin/jimenozki-htpasswd, /usr/local/bin/jimenozki-lsws" > /etc/sudoers.d/jimenozki
